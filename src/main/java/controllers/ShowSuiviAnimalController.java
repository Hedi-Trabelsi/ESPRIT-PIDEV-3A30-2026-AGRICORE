...
public class ShowSuiviAnimalController {
    ...
    private void appliquerTri() {
        ...
        switch (triCombo.getValue()) {
            case "Date (Plus récent)"        -> liste.sort(Comparator.comparing(SuiviAnimal::getDateSuivi).reversed());
            case "Date (Plus ancien)"        -> liste.sort(Comparator.comparing(SuiviAnimal::getDateSuivi));
            case "Température (Croissant)"   -> liste.sort(Comparator.comparingDouble(SuiviAnimal::getTemperature));
            case "Température (Décroissant)" -> liste.sort(Comparator.comparingDouble(SuiviAnimal::getTemperature).reversed());
            case "Poids (Croissant)"         -> liste.sort(Comparator.comparingDouble(SuiviAnimal::getPoids));
            case "Poids (Décroissant)"       -> liste.sort(Comparator.comparingDouble(SuiviAnimal::getPoids).reversed());
            case "État de santé"             -> liste.sort(Comparator.comparing(SuiviAnimal::getEtatSante));
            default -> {}
        }
    ...
    private void setupListView() {
        suiviListView.setCellFactory(param -> new ListCell<>() {
            ...
            protected void updateItem(SuiviAnimal s, boolean empty) {
                ...
                Label lblTemp   = new Label("🌡️ Temp : " + s.getTemperature() + " °C");
                Label lblPoids  = new Label("⚖️ Poids : " + s.getPoids() + " kg");
                Label lblRythme = new Label("❤️ Rythme : " + s.getRythmeCardiaque() + " bpm");
                Label lblNiveau = new Label("🏃 Niveau : " + s.getNiveauActivite());
                Label lblRem    = new Label("📝 " + s.getRemarque());
                lblRem.setWrapText(true);

                // État coloré
                Label lblEtat = new Label("🏥 État : " + s.getEtatSante());
    ...
    private void openEditWindow(SuiviAnimal s) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/UpdateSuiviAnimal.fxml"));
            Parent root = loader.load();
            UpdateSuiviAnimalController controller = loader.getController();
            controller.setSuivi(s);
            Stage stage = (Stage) suiviListView.getScene().getWindow();
    ...
    private void navigateAddSuivi() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/AddSuiviAnimal.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) suiviListView.getScene().getWindow();
            stage.getScene().setRoot(root);
        } catch (IOException e) {
