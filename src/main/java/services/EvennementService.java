...
public class EvennementService implements IService<EvennementAgricole> {
    ...
    public EvennementService() throws SQLException {
    ...
    // Optional: Add method to find by ID if needed
    public EvennementAgricole findById(int id) throws SQLException {
        String query = "SELECT * FROM evennementagricole WHERE id_ev = ?";
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ps.setInt(1, id);
