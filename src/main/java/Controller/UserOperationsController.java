...
public class UserOperationsController {
    ...
    public void setUser(User user) {
        this.user = user;
        System.out.println("[DEBUG] UserOperations - setUser id=" + (user != null ? user.getId() : "null") + " name=" + (user != null ? user.getPrenom() + " " + user.getNom() : "null"));
    ...
    private void openFinancePage() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/FinanceTables.fxml"));
    ...
    void showDepenseForm() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/DepenseForm.fxml"));
            Parent root = loader.load();
            Controller.DepenseFormController controller = loader.getController();
            controller.setUser(user);
    ...
    void showVenteForm() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/VenteForm.fxml"));
            Parent root = loader.load();
            Controller.VenteFormController controller = loader.getController();
            controller.setUser(user);
    ...
    void saveDepense() {
        try {
            if (user == null || user.getId() == 0) {
                showError("Erreur", "Utilisateur non défini (id=0). Impossible d'enregistrer.");
                return;
            }
            ...
            if (editingDepense != null) {
                ...
                if (res.isAnomaly()) {
                    ...
                    try {
                        ...
                        var css = getClass().getResource("/fxml/design.css");
                        if (css != null) {
