...
// Au survol, on garde la même finesse mais on souligne
    ...
    @FXML void showNotifications(MouseEvent event) {
        try {
            ...
            NavigationUtil.loadInContentArea(notificationBadge, root);
    ...
    @FXML void openStatsWindow(MouseEvent event) {
        try {
            ...
            NavigationUtil.loadInContentArea(statsIcon, root);
    ...
    private void openTacheWindow(Maintenance m) {
        try {
            ...
            NavigationUtil.loadInContentArea(mainList, root); // charge dans la zone de contenu
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
